...
function Handle_Operator(Next_Operator) {
    ...
    // When an operator key is pressed we convert the current number on screen to a number and store the result in Calculator.First_Operand if it doesn't already exist.
    const Value_of_Input = parseFloat(Display_Value);
    // Chekcs if an operator already exists amd if Wait_Second_Operand is true, then updates the operator and exits from the function.
    ...
    if (First_Operand == null) {
        ...
        // If operator exists, property lookup is performed for the operator in the Perform_Calculation object and the function that matches ther operator is executed.
...
keys.addEventListener('click', (event) => {
    ...
    const { target } = event;
    // If the element tha was clicked on is not a button, exit the function.
